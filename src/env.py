...
class BasePayloadEnv(gym.Env):
    ...
    def __init__(
        ...
        self.last_messages: list[tuple[float, float, float, float] | None] = [
            None for _ in range(self.n_agents)
        ]
        self.msg_age: list[int] = [999 for _ in range(self.n_agents)]
...
class PettingZooPayloadEnv(ParallelEnv):
    ...
    def get_comms(self) -> List[Optional[Tuple[float, float, float, float]]]:
        """Get communications from all agents."""
        return self.env.last_messages
