...
class BasePayloadEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        actions = decode(action, self.action_space.len, self.num_agents)
...
class PettingZooPayloadEnv(ParallelEnv):
    ...
    def __init__(self, config=None):
        ...
        config = config or {}
        self.env = BasePayloadEnv(
            n_agents=config.get("n_agents", 3),
            arena_size=config.get("arena_size", 25.0),
            goal_radius=config.get("goal_radius", 5.0),
            max_steps=config.get("max_steps", 200),
            event_driven=config.get("event_driven", True),
            cooldown_time=config.get("cooldown_time", 3),
            contact_radius=config.get("contact_radius", 3.0),
        )
        ...
        # Thought buffer for ATOC communication
        # Stores thoughts generated by each agent for others to read
        self.thought_buffer = {agent: None for agent in self.possible_agents}
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset thought buffer
        self.thought_buffer = {agent: None for agent in self.possible_agents}

        observations = {f"agent_{i}": obs[i] for i in range(len(self.agents))}
    ...
    def step(self, actions: dict) -> Tuple[Dict, Dict, Dict, Dict, Dict]:
        ...
        """
        # Convert dict to array, and mask actions for cooldown constraints
        action_array = []
        for i, agent_id in enumerate([f"agent_{i}" for i in range(len(self.agents))]):
            action = actions[agent_id]

            # In event-driven mode, if agent is on cooldown, force action to NOOP (0)
            # unless they're not on cooldown (cooldown == 0)
            if self.env.event_driven and self.env.cooldowns[i] > 0:
                # Agent is on cooldown - mask broadcast action (5) to NOOP (0)
                if isinstance(action, (int, np.integer)):
                    action = 0 if action == 5 else action
                elif isinstance(action, np.ndarray) and action.shape == ():
                    action = 0 if action == 5 else int(action)

            action_array.append(action)

        encoded_action = encode(action_array, 6)

        obs, reward, done, truncated, info = self.env.step(encoded_action)
        ...
        infos = {f"agent_{i}": info for i in range(len(self.agents))}

        # Include communication context in infos for each agent
        for i, agent in enumerate(self.agents):
            # Include readiness status (1 if ready, 0 if on cooldown)
            infos[agent]["ready"] = 1 if self.env.cooldowns[i] == 0 else 0

            # Each agent gets the thoughts from all OTHER agents
            other_agents = [a for a in self.agents if a != agent]
            other_thoughts = [
                self.thought_buffer[other_agent]
                for other_agent in other_agents
                if self.thought_buffer[other_agent] is not None
            ]
            # Store in info for RLlib to pass to model
            if other_thoughts:
                infos[agent]["other_thoughts"] = np.array(other_thoughts)
    ...
    def store_thought(self, agent_id: str, thought):
        """Store a generated thought from agent for broadcasting."""
        if agent_id in self.thought_buffer:
            self.thought_buffer[agent_id] = thought
...
def _make_parallel_pettingzoo_env(base_config: dict):
    ...
    def _creator(env_context=None):
        ...
        return PettingZooPayloadEnv(cfg)
